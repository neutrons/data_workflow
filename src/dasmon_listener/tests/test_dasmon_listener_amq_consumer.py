...
class TestAMQConsumer(unittest.TestCase):  
   ...
   def get_listener(self):
      with mock.patch('report.models.Instrument.objects') as instrumentMock:
               with mock.patch('dasmon.models.Parameter.objects') as parameterMock:
                  listener = Listener()
                  return listener
   ...
   def test_on_message_status_sts(self, jsonLoadsMock, processAckMock, storeAncCacheMock, instrumentSaveMock, parametereSaveMock):
       listener = self.get_listener()
       frame = mock.Mock()
   ...
   @mock.patch('dasmon_listener.amq_consumer.process_ack')
   @mock.patch('json.loads')
   def test_on_message_status_stc(self, jsonLoadsMock, processAckMock, storeAncCacheMock, instrumentSaveMock, parametereSaveMock):
       listener = self.get_listener()
       frame = mock.Mock()
       #ending with .ACK returns premptively
   ...
   @mock.patch('dasmon_listener.amq_consumer.process_ack')
   @mock.patch('json.loads')
   def test_on_message_status_sms(self, jsonLoadsMock, processAckMock, storeAncCacheMock, instrumentSaveMock, parametereSaveMock):
       listener = self.get_listener()
       frame = mock.Mock()
       #ending with .ACK returns premptively
   ...
   @mock.patch('dasmon_listener.amq_consumer.process_ack')
   @mock.patch('json.loads')
   def test_on_message_status_dict(self, jsonLoadsMock, processAckMock, storeAncCacheMock, instrumentSaveMock, parametereSaveMock):
       listener = self.get_listener()
       frame = mock.Mock()
       #ending with .ACK returns premptively
       frame.headers = {'header1':'header1_value', 'destination':'STATUS.'}
       frame.body = 'body'
       jsonLoadsMock.return_value = {"status":"cowboy2", "src_id":"eels", "pid":"norman reedus"}
       listener.on_message(frame)
       jsonLoadsMock.assert_called()
       storeAncCacheMock.assert_called()
   ...
   def test_on_message_status_else(self, jsonLoadsMock, storeAncCacheMock, instrumentSaveMock, parametereSaveMock):
       ...
       frame.body = 'body'
       jsonLoadsMock.return_value = {"status":"cowboy2", "monitors" : {"item1":"item1", "dict1":{"id":"dict1_id", "counts":"dict1_counts"}}}
       listener.on_message(frame)
       jsonLoadsMock.assert_called()
       storeAncCacheMock.assert_called()
   ...
   def test_store_and_cache_str_len(self, statusVariableMock, statusCacheMock):
        ...
        value = 'the max string length is one-hundred and twenty eight, this string should be longer than that and cause it to cut off the end of this sentence'
        store_and_cache_(instrument_id, key_id, value, None, False)
        statusVariableMock.assert_called_with(instrument_id=instrument_id, key_id=key_id , value='the max string length is one-hundred and twenty eight, this string should be longer than that and cause it to cut off the end of')
