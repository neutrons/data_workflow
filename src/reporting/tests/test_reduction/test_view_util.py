...
class TestViewUtil(TestCase):
    ...
    def setUpTestData(cls):
        ...
        rp = ReductionProperty.objects.create(instrument=inst,
                                              key='key',
                                              value='value')
        PropertyDefault.objects.create(property=rp,
                                       value='default')
    ...
    def test_store_propertry(self):
        ...
        ReductionProperty.objects.create(instrument=inst,
                                         key='new_key',
                                         value='new_value2')
        view_util.store_property(inst, 'new_key', 'new_value3', user)

        rp = ReductionProperty.objects.filter(instrument=inst,
                                              key='new_key')
    ...
    def test_send_template_request(self, mock_activemq, mock_dasmon_entry):
        ...
        view_util.send_template_request(inst,
                                        {'input1': 'value1',
                                         'input2': 2,
                                         'use_default': True})

        mock_activemq.assert_called_with('/queue/REDUCTION.CREATE_SCRIPT',
                                         '{"instrument": "INST", "use_default": true, '
                                         '"template_data": {"input1": "value1", "input2": 2, "use_default": true}, '
                                         '"information": "Requested by unknown"}')

        mock_dasmon_entry.assert_called_with(inst,
                                             'system_postprocessing',
                                             'Script requested by unknown')
