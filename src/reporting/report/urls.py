...
urlpatterns = [
    ...
    url(r'^(?P<instrument>[\w]+)/(?P<run_id>\d+)/postprocess/$',
        views.submit_for_post_processing, name='submit_for_post_processing'),
    url(r'^(?P<instrument>[\w]+)/experiment/(?P<ipts>[\w\-\.]+)/update/$',
        views.get_experiment_update, name='get_experiment_update'),
